function SimuRslt = simulate_model(IterRslt,GNDSGE_OPTIONS)
%% Add path
if ispc
...
for GNDSGE_t=1:num_periods
    ...
    GNDSGE_data0 = repmat([shock_num;beta(:);sigma(:);alpha(:);delta(:);phi(:);Iss(:);shock_trans(:);z(:); ],1,GNDSGE_NPROB);
...
SimuRslt.K(:,GNDSGE_t+1) = K_next(:);
    ...
    if mod(GNDSGE_t,SimuSaveFreq)==0
        save(['SimuRslt_model_' num2str(GNDSGE_t) '.mat'], 'SimuRslt');
    end
end
end