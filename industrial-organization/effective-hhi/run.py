...
def main():
    ...
    report.add_overview(
        ...
        "in a relevant market? It is cheap to compute, transparent to explain, and useful as "
        "an early merger screen. The same simplicity is its limitation. HHI knows ownership "
        "shares, not diversion ratios, entry, efficiencies, or demand curvature.\n\n"
        "The tutorial keeps that distinction explicit. The first part works through the "
...
"""
    ...
    report.add_solution_method(
        ...
        "way to solve a small system of first-order conditions; the economic content of the "
        "tutorial is elsewhere. HHI is an ownership screen, while the pricing effect appears "
        "only through substitution and the Bertrand FOC."
    )
    ...
    report.add_results(
        ...
        "With positive cross-price substitution, common ownership changes the pricing "
    ...
    report.add_takeaway(
        "HHI is transparent: it converts shares into a concentration number and gives a "
        "closed-form delta for mergers. The segmented-product example is the warning label. "
        "Ownership aggregation can raise HHI even when the maintained demand model implies no "
        "price effect. Once products substitute, the same ownership change works through the "
        "Bertrand FOC and prices move. In applied work, HHI should start the antitrust "
