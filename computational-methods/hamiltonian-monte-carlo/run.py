...
"""
    ...
    report.add_solution_method(
        ...
        "Random-walk Metropolis-Hastings is included as the baseline because the difference is exactly the point of HMC.\n\n"

        "### Method 1: Leapfrog integration\n\n"
        "Leapfrog is the standard symplectic integrator for Hamiltonian systems. "
        "It interleaves momentum half-steps with position full-steps. "
        "The order matters: two half-steps in the momentum bracket one position step, so the integrator is time-reversible at each substep and conserves volume in $(\\theta, r)$-space. "
        ...
        "The number of steps $L$ controls trajectory length, $L\\,\\varepsilon$. "
        "Long trajectories explore the posterior aggressively but at higher cost per iteration and with a risk of trajectory U-turn, which is the motivation for the No-U-Turn Sampler that automates $L$.\n\n"

        "### Method 2: Hamiltonian Monte Carlo iteration\n\n"
        "Each HMC iteration is a momentum resample plus a leapfrog trajectory plus a Metropolis accept-reject. "
        ...
        "The Metropolis step is necessary because leapfrog conserves the Hamiltonian only approximately, but on a well-tuned trajectory the error is tiny and acceptance rates of 0.7 to 0.9 are typical.\n\n"
        "```text\n"
