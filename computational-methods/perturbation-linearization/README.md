...
To see how the first-order coefficient drops out of the Taylor expansion and what it costs away from the steady state, take $`\rho = 0.82`$, $`\gamma = 0.45`$, $`\eta = 0.80`$, $`\kappa = 0.35`$, and shock $`\epsilon = 0.18`$.

Steady state is $`x^{\ast} = 0`$ by construction, so the Taylor coefficients are derivatives of $`F`$ evaluated at zero:

```math
F'(0) = \rho = 0.82, \qquad \tfrac{1}{2} F''(0) = \gamma = 0.45, \qquad \tfrac{1}{6} F'''(0) = -\eta = -0.80.
```

The first-order law is $`F_1(x) = \rho x`$, so the linearized one-step response to the shock is

```math
\boxed{x_1 = \rho \, \epsilon = (0.82)(0.18) = 0.1476.}
```
