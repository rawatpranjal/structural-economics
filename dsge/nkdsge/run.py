...
def main():
    ...
    report.add_overview(
        "A central bank raises the policy rate when firms adjust prices slowly. The nominal "
        "surprise raises the real rate before prices catch up. Demand falls, and the Phillips "
        "curve turns the lower output gap into lower inflation.\n\n"
        "The model has three variables: output gap $y_t$, inflation $\\pi_t$, and policy rate "
        "$i_t$. The shocks are a Taylor-rule wedge $v_t$ and a natural-rate demand shock $d_t$. "
        "The tutorial studies their impulse responses.\n\n"
        "Today's output and inflation depend on expected future values, so the equilibrium is "
        "forward looking. Because the system is log-linear, coefficient matching solves the "
        "shock loadings directly. A Klein QZ solve checks the same stable path."
    )
...
"""
    ...
    report.add_model_setup(
        ...
        "instead uses $\\phi_\\pi=1.5$ and $\\kappa=0.3$. That calibration keeps the "
        "Taylor rule active enough to select one stable path."
    )

    report.add_solution_method(
        "Let the active shock be $s_t=\\rho_s s_{t-1}+\\varepsilon_t$. The equilibrium "
        "object is a pair of coefficients. They map the shock state into output and "
        "inflation:\n\n"
        "$$y_t=\\psi_y s_t,\\qquad \\pi_t=\\psi_\\pi s_t. $$\n\n"
        "The Phillips curve links the two coefficients:\n\n"
        "$$\\psi_\\pi=\\frac{\\kappa\\psi_y}{1-\\beta\\rho_s}. $$\n\n"
        "Substituting the guess into the IS curve and Taylor rule leaves one scalar "
        "equation:\n\n"
        "$$\\psi_y\\left[(1-\\rho_s)+\\frac{\\phi_y}{\\sigma}"
        "+\\frac{(\\phi_\\pi-\\rho_s)\\kappa}{\\sigma(1-\\beta\\rho_s)}\\right]"
        "= b_s,$$\n\n"
        "Here $b_s=-1/\\sigma$ for a policy wedge and $b_s=1$ for a demand shock. "
        "The sign changes because the two shocks enter different equations.\n\n"
        "```text\n"
        ...
        "```\n\n"
        "Klein QZ solves the same linear system as a check. The coefficient solution "
        f"and QZ solution differ by at most {max(mp_qz_diff, d_qz_diff):.1e}. This "
        "confirms the stable rational-expectations equilibrium for this calibration."
    )
    ...
    report.add_figure(
        ...
        fig1,
        description="The monetary shock is the Taylor-rule wedge, not the full policy-rate response. "
        "The wedge raises the real rate on impact. Output and inflation fall, so the systematic part "
        "of the rule partly offsets the wedge.",
    )
    ...
    report.add_figure(
        ...
        fig2,
        description="A positive natural-rate shock raises current demand at a given nominal rate. "
        "Output and inflation rise together. The Taylor rule raises the policy rate, which dampens "
        "the expansion.",
    )
    ...
    report.add_table(
        ...
        df,
        description="The table reports impact signs and sizes. Output is in percent deviations. "
        "Inflation and the policy rate are quarterly percentage points. The two shocks move output "
        "and inflation in opposite directions.",
    )

    report.add_takeaway(
        "The three-equation New Keynesian model shows how sticky prices make nominal "
        "policy matter. A policy wedge raises the real rate and contracts demand. A "
        "natural-rate shock expands demand and inflation, with the Taylor rule leaning back.\n\n"
        "Coefficient matching is enough because the model is log-linear. The Klein QZ "
        "check confirms the same stable equilibrium. Determinacy is economic here: "
        "inflation feedback selects one forward-looking path."
    )
