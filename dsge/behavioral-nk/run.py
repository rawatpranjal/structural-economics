...
"""
    ...
    report.add_results(
        ...
        "curve. Cognitive discounting breaks part of that backward chain, so the "
        "behavioral response is smaller than the rational one at most horizons. "
        "It is not smaller everywhere. At a zero-quarter horizon the wedge is "
        "contemporaneous, there is nothing to discount, and the two models tie "
        "exactly. Near the sign-change band the rational and behavioral curves "
        "cross sign at different dates, so a short window around a five-quarter "
        "horizon has the behavioral absolute response slightly above the rational "
        "one at very small magnitudes. "
        "The shape is not a simple monotone decay, though. Both models trace a "
        "hump: the absolute date-0 response falls to a local minimum near a "
        "six-quarter horizon, then rises again before declining at long horizons. "
        "The rise is a residual forward-guidance puzzle. The signed date-0 output "
        "response changes sign at moderate horizons, so a future rate hike can "
        "raise current output rather than lower it. Cognitive discounting at "
        "M = M_f = 0.85 shrinks this puzzle but does not remove it; the sign "
        "reversal still appears in the behavioral model from a seven-quarter "
        "horizon onward. The clean result is the level gap, not monotone decay: "
        "distant forward guidance is much weaker under behavioral attention than "
