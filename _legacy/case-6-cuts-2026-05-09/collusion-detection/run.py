...
def main():
    ...
    report.add_overview(
        "Cartel investigations often start with a price path. Prices rise together "
        "and stay high. They fall after detection or a breakdown.\n\n"
        "The object is a repeated agreement among Cournot firms. Each firm weighs "
        "future cartel profit against today's gain from extra output.\n\n"
        "The computation compares Nash, collusive, and deviation profits. Those "
        "payoffs give the discount-factor threshold and benchmarks for a simulated "
        "price screen."
    )
...
"""
    ...
    report.add_model_setup(
        "The calibration is small enough to audit by hand. Demand and cost pin down "
        "Nash and monopoly prices. The simulated data add only regime timing and "
        "noise.\n\n"
        f"| Object | Value | Role |\n"
        f"|---|---:|---|\n"
        ...
        f"In the baseline duopoly, the Cournot price is {P_compete:.0f}. "
        f"The monopoly price is {P_collude:.0f}. "
        f"A deviating firm earns {pi_deviate:.1f} when its rival holds the cartel quantity."
    )

    report.add_solution_method(
        "First compute payoffs for each firm count. Then evaluate the repeated-game "
        "threshold. For the duopoly, simulate prices around Nash, monopoly, then "
        "Nash again. The margin series uses those prices and constant cost.\n\n"
        "```text\n"
        ...
        "```\n\n"
        f"With $\\delta={delta_reference:.1f}$, the threshold allows at most "
        f"{max_n_delta} symmetric firms."
    )
    ...
    report.add_figure(
        ...
        f"At $\\delta={delta_reference:.1f}$, {max_n_delta} firms are still sustainable. "
        f"At {max_n_delta + 1} firms, the deviation constraint fails.",
    )
    ...
    report.add_figure(
        ...
        fig3,
        description="The simulated price path has known regimes. Prices begin near the "
        "Nash benchmark, move toward monopoly, then return to Nash. This is a clean "
        "benchmark, not a detection test.",
    )
    ...
    report.add_figure(
        ...
        fig4,
        description="The margin plot repeats the same break after normalizing by price. "
        "Because cost is constant, it adds no new identification in this toy run.",
    )
    ...
    # --- Economic takeaway ---
    report.add_takeaway(
        "A price break is a lead, not a cartel finding. The repeated-game check asks "
        "whether future cartel rents can deter one-period cheating. "
        f"In the duopoly, $\\delta^{{*}}={delta_star_2:.4f}$. "
        f"With ten symmetric firms, $\\delta^{{*}}={critical_discount_factor(10, a, c):.4f}$. "
        f"At $\\delta={delta_reference:.1f}$, the exact firm-count cutoff is "
        f"{max_n_delta}. Cost and demand evidence still matters."
    )
