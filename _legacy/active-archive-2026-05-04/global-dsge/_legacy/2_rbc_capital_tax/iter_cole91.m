function [IterRslt,IterFlag] = iter_cole91(GNDSGE_OPTIONS)
%% Add path
if ispc
...
GNDSGE_DATA(:) = [repmat([shock_num;beta(:);sigma(:);alpha(:);delta(:);tau(:);shock_trans(:);z(:); ],1,GNDSGE_NPROB);GNDSGE_TENSOR_shockIdx(:)';GNDSGE_TENSOR_K(:)';   ];
...
[GNDSGE_SOL,GNDSGE_F,GNDSGE_AUX,GNDSGE_EQVAL,GNDSGE_OPT_INFO] = mex_cole91(GNDSGE_SOL,GNDSGE_LB,GNDSGE_UB,GNDSGE_DATA,GNDSGE_SKIP,GNDSGE_F,GNDSGE_AUX,GNDSGE_EQVAL);
...
IterRslt.output_var_index = output_var_index;
        ...
        IterRslt.params = v2struct(beta,sigma,alpha,delta,tau,GNDSGE_EMPTY);
        ...
        if ~NoSave
            if IterSaveAll
                save(['IterRslt_cole91_' num2str(GNDSGE_Iter) '.mat']);
            else
                save(['IterRslt_cole91_' num2str(GNDSGE_Iter) '.mat'],'IterRslt');
            end
        end
    end
end
