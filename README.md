...
| [<img src="heterogeneous-agents/sequence-space-jacobian-hank/figures/thumb.png" width="160">](heterogeneous-agents/sequence-space-jacobian-hank/figures/irf-comparison.png) | **[Sequence-Space Jacobian for One-Asset HANK](heterogeneous-agents/sequence-space-jacobian-hank/)** | Solve a one-asset HANK economy's response to a 25 basis-point monetary shock by linearizing each block in sequence space. The fake-news algorithm builds the household-block Jacobian from one backward EGP step plus a forward distribution propagation, and the firm, NKPC, and Taylor blocks compose into a single linear solve. |
...
| [<img src="computational-methods/simulation-based-estimation/figures/thumb.png" width="160">](computational-methods/simulation-based-estimation/figures/criterion-surfaces.png) | **[Estimating a Search Acceptance Rule by Simulation](computational-methods/simulation-based-estimation/)** | Estimate a reservation-wage rule from offers and acceptances. MSM matches economic moments, indirect inference matches an auxiliary model, and ABC-SMC samples a posterior from level sets of the same scaled distance. |
| [<img src="structural-econometrics/adversarial-estimation/figures/thumb.png" width="160">](structural-econometrics/adversarial-estimation/figures/smm-vs-adversarial.png) | **[Adversarial Structural Estimation](structural-econometrics/adversarial-estimation/)** | Drop hand-picked moments and let a discriminator decide. A logistic discriminator recovers optimally-weighted SMM; a small neural discriminator stays close to MLE while SMM precision collapses as the moment count grows. |
...
| [<img src="game-theory/cfr-asymmetric-auction/figures/thumb.png" width="160">](game-theory/cfr-asymmetric-auction/figures/bid-functions-asymmetric.png) | **[Asymmetric First-Price Auctions by Counterfactual Regret Minimization](game-theory/cfr-asymmetric-auction/)** | Two bidders draw values from different uniform supports, so no closed-form Bayesian Nash equilibrium is available. Vanilla CFR and CFR+ run regret matching at every type and converge to the BNE; exploitability of the average strategy on the bid grid is the deviation-check diagnostic, and the symmetric uniform case anchors the implementation against the $v / 2$ closed form. |
