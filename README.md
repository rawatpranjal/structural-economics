...
| [<img src="numerical-methods/quadrature/figures/thumb.png" width="160">](numerical-methods/quadrature/figures/error-vs-nodes.png) | **[Numerical Quadrature: Gauss-Hermite Nodes for Conditional Expectations](numerical-methods/quadrature/)** | Gauss-Hermite nodes integrate against the Gaussian weight in closed form, converging geometrically for smooth integrands. A change of variables turns AR(1) conditional expectations into a small weighted sum that beats Monte Carlo by orders of magnitude on the same compute budget. |
| [<img src="numerical-methods/neural-networks-regression/figures/thumb.png" width="160">](numerical-methods/neural-networks-regression/figures/fitted-surface.png) | **[Feedforward Neural Networks for Regression and Density Approximation](numerical-methods/neural-networks-regression/)** | A one-hidden-layer tanh network fits a Cobb-Douglas surface with Gaussian noise. Forward pass, JAX autodiff for backprop, L2 weight decay, and Adam are the four primitives that every neural utility, discriminator, and mechanism in the catalog reuses. |
...
| [<img src="numerical-methods/simulated-likelihood/figures/thumb.png" width="160">](numerical-methods/simulated-likelihood/figures/bias-variance-vs-R.png) | **[Simulated Maximum Likelihood, Common Random Numbers, and Halton Sequences](numerical-methods/simulated-likelihood/)** | Replace an intractable integrated likelihood by a finite average over fixed draws. Common random numbers make the objective smooth; Halton and scrambled Sobol sequences cut bias at the same draw count by filling space more uniformly than pseudo-random points. |
...
| [<img src="computational-methods/smolyak-sparse-grids/figures/thumb.png" width="160">](computational-methods/smolyak-sparse-grids/figures/grid-size-scaling.png) | **[High-Dimensional Growth Policy by Smolyak Sparse Grids](computational-methods/smolyak-sparse-grids/)** | A multi-sector planner with five continuous states defeats tensor Chebyshev grids by node count alone. Smolyak collocation recovers the closed-form saving rule at a fraction of the cost. |
...
| Preview | Tutorial | Description |
|---|---|---|
| [<img src="heterogeneous-agents/kolmogorov-forward-equation/figures/thumb.png" width="160">](heterogeneous-agents/kolmogorov-forward-equation/figures/stationary-density-ou.png) | **[Kolmogorov Forward Equation and the Stationary Wealth Distribution](heterogeneous-agents/kolmogorov-forward-equation/)** | Mass conservation gives the KFE for the cross-sectional density. The discretised forward operator is the transpose of the upwind HJB generator. A sparse linear solve plus a normalisation row delivers the stationary distribution. |
| [<img src="heterogeneous-agents/endogenous-grid-points/figures/thumb.png" width="160">](heterogeneous-agents/endogenous-grid-points/figures/consumption-policy.png) | **[Buffer-Stock Saving with IID Income by EGP](heterogeneous-agents/endogenous-grid-points/)** | Solve a buffer-stock saving problem under IID income without an inner asset-choice search. EGP works backward from next-period assets through the Euler equation. |
...
| [<img src="heterogeneous-agents/aiyagari-hact/figures/thumb.png" width="160">](heterogeneous-agents/aiyagari-hact/figures/capital-market.png) | **[Continuous-time Aiyagari and the Mean-Field Game](heterogeneous-agents/aiyagari-hact/)** | Extend Huggett to a production economy with capital and labor. The HJB, KFE, and capital-market clearing form a Lasry-Lions mean-field game; a discrete-time Aiyagari solver runs alongside. |
| [<img src="heterogeneous-agents/huggett-aggregate-risk-srl/figures/thumb.png" width="160">](heterogeneous-agents/huggett-aggregate-risk-srl/figures/policy-consumption.png) | **[Structural Reinforcement Learning for Huggett with Aggregate Risk](heterogeneous-agents/huggett-aggregate-risk-srl/)** | Households save under idiosyncratic and aggregate risk while the interest rate clears the bond market. A structural policy-gradient algorithm learns price-conditioned saving rules. |
| [<img src="heterogeneous-agents/sequence-space-jacobian-hank/figures/thumb.png" width="160">](heterogeneous-agents/sequence-space-jacobian-hank/figures/irf-comparison.png) | **[Sequence-Space Jacobian for One-Asset HANK](heterogeneous-agents/sequence-space-jacobian-hank/)** | Solve a one-asset HANK economy's response to a monetary shock by linearizing each block in sequence space. Fake-news Jacobians compose into a single linear solve. |
...
| Preview | Tutorial | Description |
|---|---|---|
| [<img src="dsge/blanchard-kahn-determinacy/figures/thumb.png" width="160">](dsge/blanchard-kahn-determinacy/figures/bk-heatmap.png) | **[Blanchard-Kahn Conditions and Saddle-Path Selection in Linear Rational Expectations](dsge/blanchard-kahn-determinacy/)** | A 2x2 forward-looking RE model is solved by counting stable generalised eigenvalues against predetermined states. The same QZ partition runs on a small DSGE and selects the unique non-explosive path. |
| [<img src="dsge/rbc/figures/thumb.png" width="160">](dsge/rbc/figures/irf-fixed-labor.png) | **[Linearized RBC by Perturbation and QZ (with and without endogenous labor)](dsge/rbc/)** | A TFP shock propagates through investment and (when labor is endogenous) hours. Klein QZ solves the linearized policy, checked against the exact nonlinear transition. |
...
| Preview | Tutorial | Description |
|---|---|---|
| [<img src="optimal-control/upwind-finite-differences/figures/thumb.png" width="160">](optimal-control/upwind-finite-differences/figures/value-and-drift.png) | **[Upwind Finite Differences for First-Order PDEs and the HJB State Constraint](optimal-control/upwind-finite-differences/)** | A drift-driven 1D HJB on a bounded interval is discretised with one-sided finite differences. Sign of the policy-implied drift selects forward or backward, and a Kuhn-Tucker clip enforces the state constraint at the boundary. |
...
| [<img src="industrial-organization/bertrand-ownership-matrix/figures/thumb.png" width="160">](industrial-organization/bertrand-ownership-matrix/figures/prices-pre-post.png) | **[Multi-Product Bertrand-Nash Pricing and the Ownership Matrix](industrial-organization/bertrand-ownership-matrix/)** | Multi-product firms internalize cross-price effects across the products they own. The Hadamard product of ownership and demand Jacobian stacks the FOC into one linear system that solves for equilibrium prices, recovers marginal costs, and produces post-merger counterfactual prices. |
...
| [<img src="industrial-organization/online-pricing-partial-identification/figures/thumb.png" width="160">](industrial-organization/online-pricing-partial-identification/figures/active-prices.png) | **[Online Pricing with Revealed-Preference Bounds](industrial-organization/online-pricing-partial-identification/)** | A seller posts prices on a grid and observes only buy or no-buy. WARP-style monotonicity turns each observation into a per-segment valuation bound, dominated prices are eliminated, and the active price set shrinks from twenty to three within the first hundred rounds under UCB-PI. |
...
| Preview | Tutorial | Description |
|---|---|---|
| [<img src="structural-econometrics/gmm-foundations/figures/thumb.png" width="160">](structural-econometrics/gmm-foundations/figures/efficiency-gain.png) | **[GMM Foundations: Moment Conditions, Identification, and Optimal Weighting](structural-econometrics/gmm-foundations/)** | A location-model GMM estimator runs from a moment condition through Hansen's two-step optimal weighting. Adding moments raises efficiency only when each one adds independent identifying variation; redundant moments shrink the gain. |
| [<img src="industrial-organization/dynamic-discrete-choice/figures/thumb.png" width="160">](industrial-organization/dynamic-discrete-choice/figures/value-and-ccp.png) | **[Bus Engine Replacement: NFXP, CCP, MPEC, and the MCE-IRL Equivalence](industrial-organization/dynamic-discrete-choice/)** | Study bus engine replacement when mileage changes operating costs. NFXP, CCP, MPEC, and maximum-causal-entropy IRL all recover the same replacement hazard. |
...
| [<img src="computational-methods/simulation-based-estimation/figures/thumb.png" width="160">](computational-methods/simulation-based-estimation/figures/criterion-surfaces.png) | **[Estimating a Search Acceptance Rule by Simulation](computational-methods/simulation-based-estimation/)** | Estimate a reservation-wage rule from offers and acceptances. MSM matches moments, indirect inference matches an auxiliary model, and ABC-SMC samples a posterior from the same scaled distance. |
| [<img src="structural-econometrics/adversarial-estimation/figures/thumb.png" width="160">](structural-econometrics/adversarial-estimation/figures/smm-vs-adversarial.png) | **[Adversarial Structural Estimation](structural-econometrics/adversarial-estimation/)** | Drop hand-picked moments and let a discriminator choose. A logistic discriminator recovers optimally-weighted SMM; a neural discriminator stays close to MLE as the moment count grows. |
...
| [<img src="structural-econometrics/bayesian-dsge-hmc/figures/thumb.png" width="160">](structural-econometrics/bayesian-dsge-hmc/figures/posterior-irfs.png) | **[Bayesian DSGE Estimation by HMC/NUTS](structural-econometrics/bayesian-dsge-hmc/)** | Estimate a small two-shock New Keynesian DSGE from simulated data. JAX-differentiable Klein QZ and Kalman likelihood feed BlackJAX NUTS; the posterior concentrates on the data-generating parameters and NUTS dominates random-walk Metropolis in effective sample size. |
...
| Preview | Tutorial | Description |
|---|---|---|
| [<img src="bayesian-methods/bayesian-foundations/figures/thumb.png" width="160">](bayesian-methods/bayesian-foundations/figures/beta-posteriors.png) | **[Bayesian Foundations: Priors, Likelihoods, and Conjugate Posteriors](bayesian-methods/bayesian-foundations/)** | Bayes rule turns a prior into a posterior. Beta-Binomial conjugacy gives a closed-form scalar update; Gaussian-Gaussian conjugacy gives a closed-form linear regression update with shrinkage as a precision-weighted average. |
| [<img src="computational-methods/metropolis-hastings/figures/thumb.png" width="160">](computational-methods/metropolis-hastings/figures/conjugate-posterior.png) | **[Posterior Sampling: Random-walk Metropolis-Hastings](computational-methods/metropolis-hastings/)** | Run random-walk Metropolis-Hastings on a two-regime mixture posterior. The Beta-Binomial conjugate model is kept as a closed-form sanity check against the chain output. |
| [<img src="computational-methods/mcmc-diagnostics/figures/thumb.png" width="160">](computational-methods/mcmc-diagnostics/figures/trace-plots.png) | **[MCMC Chain Diagnostics: ESS, R-hat, and Integrated Autocorrelation Time](computational-methods/mcmc-diagnostics/)** | A correlated-Gaussian target is sampled with random-walk Metropolis. ESS, IAT, R-hat across multiple chains, and trace plots each catch a different pathology before any structural model is touched. |
...
| [<img src="computational-methods/hamiltonian-monte-carlo/figures/thumb.png" width="160">](computational-methods/hamiltonian-monte-carlo/figures/posterior-coverage.png) | **[Hamiltonian Monte Carlo on a Banana Posterior](computational-methods/hamiltonian-monte-carlo/)** | Replace the random-walk proposal with leapfrog Hamiltonian dynamics. On a curved banana ridge HMC delivers an order-of-magnitude better effective sample size than random-walk MH. |
| [<img src="bayesian-methods/neural-posterior-brock-hommes/figures/thumb.png" width="160">](bayesian-methods/neural-posterior-brock-hommes/figures/posterior-pairs.png) | **[Likelihood-Free Bayes by Neural Posterior Estimation](bayesian-methods/neural-posterior-brock-hommes/)** | Train a normalizing flow on simulator output and evaluate the posterior at observed summary statistics. On the Brock-Hommes ABM the same flow returns a four-parameter joint posterior without writing a likelihood or running a sampler. |
| [<img src="numerical-methods/gaussian-processes/figures/thumb.png" width="160">](numerical-methods/gaussian-processes/figures/posterior-fit.png) | **[Gaussian Process Regression and Uncertainty Quantification](numerical-methods/gaussian-processes/)** | A Gaussian process places a prior over functions; conditioning on noisy data returns a closed-form posterior mean and credible band. Marginal likelihood tunes the kernel length scale; the same GP is the surrogate behind Bayesian optimization. |
| [<img src="numerical-methods/bayesian-optimization/figures/thumb.png" width="160">](numerical-methods/bayesian-optimization/figures/bo-iterations.png) | **[Bayesian Optimization with a Gaussian-Process Surrogate](numerical-methods/bayesian-optimization/)** | Optimize a two-segment monopoly profit with a Gaussian-process surrogate and Expected Improvement. Thirty evaluations recover the global peak that simulated annealing needs thousands to certify. |
...
| [<img src="choice/weitzman-search-rule/figures/thumb.png" width="160">](choice/weitzman-search-rule/figures/reservation-values.png) | **[Pandora's Box: Optimal Sequential Search and the Weitzman Reservation-Value Rule](choice/weitzman-search-rule/)** | A buyer inspects boxes with random values at a per-box cost. Weitzman's reservation value $`z_j`$ ranks each box by the option value of inspection; opening in decreasing $`z_j`$ under perfect recall is exactly optimal. |
...
| [<img src="game-theory/regret-matching/figures/thumb.png" width="160">](game-theory/regret-matching/figures/time-average-convergence.png) | **[Regret Matching and No-Regret Dynamics](game-theory/regret-matching/)** | Each player plays in proportion to positive cumulative regret. The time-average strategy converges to a correlated equilibrium without any opponent model, and the same primitive scales up to CFR for extensive-form games. |
| [<img src="game-theory/cfr-asymmetric-auction/figures/thumb.png" width="160">](game-theory/cfr-asymmetric-auction/figures/bid-functions-asymmetric.png) | **[Asymmetric First-Price Auctions by Counterfactual Regret Minimization](game-theory/cfr-asymmetric-auction/)** | Two bidders draw values from different uniform supports, so no closed-form BNE is available. Vanilla CFR runs regret matching at each type and converges to the BNE; the same equilibrium is recovered independently from the Marshall-Meurer-Richard-Stromquist ODE on the inverse bid functions, and the two computations agree within bid-grid spacing. |
...
| [<img src="time-series/reduced-form-var/figures/thumb.png" width="160">](time-series/reduced-form-var/figures/irf-by-ordering.png) | **[Reduced-Form VARs and Cholesky Impulse Responses](time-series/reduced-form-var/)** | Estimate a bivariate VAR by ordinary least squares and identify shocks under two Cholesky orderings. Impulse responses depend on the ordering; estimation error shrinks at the sqrt-N rate. |
