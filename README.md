...
|---|---|---|
| [<img src="dynamic-programming/shock-discretization/figures/thumb.png" width="160">](dynamic-programming/shock-discretization/figures/stationary-mass.png) | **[Discretizing Persistent Shocks](dynamic-programming/shock-discretization/)** | Start from persistent income or productivity risk, then ask what finite shock process the Bellman equation should see. Tauchen turns the AR(1) into visible Gaussian cells; Rouwenhorst is less visual but preserves variance and persistence on coarse grids. |
| [<img src="dynamic-programming/cake-eating/figures/thumb.png" width="160">](dynamic-programming/cake-eating/figures/value-function.png) | **[Finite-Resource Cake Eating](dynamic-programming/cake-eating/)** | The cleanest dynamic programming benchmark: a non-renewable stock consumed over an infinite horizon. Log utility delivers a closed form, so VFI can be audited pointwise against the Hotelling-style Euler rule $u'(c_t)=\beta u'(c_{t+1})$. |
| [<img src="dynamic-programming/optimal-growth/figures/thumb.png" width="160">](dynamic-programming/optimal-growth/figures/value-function.png) | **[Optimal Growth by Value Function Iteration](dynamic-programming/optimal-growth/)** | The Ramsey-Cass-Koopmans planner with one capital stock. Log utility, Cobb-Douglas production, and full depreciation collapse the optimal saving rate to $\alpha\beta$, so VFI can be audited pointwise against the closed-form transition to $k_{ss}=(\alpha\beta A)^{1/(1-\alpha)}$. |
| [<img src="dynamic-programming/solow-growth/figures/thumb.png" width="160">](dynamic-programming/solow-growth/figures/solow-diagram.png) | **[Solow Growth and Conditional Convergence](dynamic-programming/solow-growth/)** | Iterate the Solow map for capital per effective worker and audit it against the closed-form steady state. Saving and depreciation set the level of $k^{\ast}$; long-run growth comes from labor-augmenting technology. |
| [<img src="dynamic-programming/consumption-savings/figures/thumb.png" width="160">](dynamic-programming/consumption-savings/figures/value-functions.png) | **[Income Risk and Buffer-Stock Saving](dynamic-programming/consumption-savings/)** | The household block of incomplete-markets macro: persistent income risk, a no-borrowing constraint, and CRRA preferences. With $\beta R<1$, prudence pulls assets up to a stochastic buffer-stock target, and the median-income MPC falls from about $0.5$ near the constraint to a few percent deep in the wealth distribution. |
| [<img src="dynamic-programming/job-search-mccall/figures/thumb.png" width="160">](dynamic-programming/job-search-mccall/figures/accept-vs-reject.png) | **[McCall Job Search and the Reservation Wage](dynamic-programming/job-search-mccall/)** | The cleanest optimal-stopping problem in the catalog. Because the rejection value is independent of the current offer, the Bellman equation collapses to a scalar fixed point in $w^{\ast}$, and finite-grid VFI can be audited against a closed-form lognormal benchmark. Patience and benefits both raise the cutoff; the right tail keeps it well above $\mathbb{E}[W]$. |
| [<img src="dynamic-programming/asset-pricing/figures/thumb.png" width="160">](dynamic-programming/asset-pricing/figures/asset-price-function.png) | **[Lucas Tree Prices and the Stochastic Discount Factor](dynamic-programming/asset-pricing/)** | Market clearing forces $c_t=y_t$, so the entire model lives inside the equilibrium price. Multiplying the Euler equation by $u'(y)$ turns it into a linear $\beta$-contraction in the scaled price $f(y)=u'(y)p(y)$; log utility collapses $p/y$ to $\beta/(1-\beta)$, and any state-dependence in the price-dividend ratio is preferences interacting with mean reversion in dividends. |
| [<img src="dynamic-programming/rbc/figures/thumb.png" width="160">](dynamic-programming/rbc/figures/comovements.png) | **[RBC Capital, Labor, and Business-Cycle Moments](dynamic-programming/rbc/)** | A representative-household RBC model with endogenous labor and persistent two-state TFP, solved by global VFI on a capital-TFP grid rather than log-linearized. Investment carries about $4\times$ the volatility of output, consumption is smoother by a factor of three, and hours are strongly procyclical — the canonical King-Plosser-Rebelo signature, recovered without leaving the nonlinear policy functions. |
| [<img src="dynamic-programming/diamond-mortensen-pissarides/figures/thumb.png" width="160">](dynamic-programming/diamond-mortensen-pissarides/figures/productivity-tightness.png) | **[DMP Search, Vacancies, and Unemployment](dynamic-programming/diamond-mortensen-pissarides/)** | Free entry of vacancies turns expected match surplus into labor-market tightness $\theta_t$, and the matching function does the rest. A linearized local rule and a global free-entry fixed point on a Rouwenhorst grid agree to four decimal places, so the Shimer (2005) amplification gap — $\sigma_\theta/\sigma_z\approx 1.7$ vs roughly $19$ in the data — is *economic*: shrinking the surplus $\bar z-b$ in the Hagedorn-Manovskii direction is what closes it. |
| [<img src="dynamic-programming/aiyagari/figures/thumb.png" width="160">](dynamic-programming/aiyagari/figures/capital-market.png) | **[Aiyagari Saving and Capital-Market Clearing](dynamic-programming/aiyagari/)** | Aggregate household precautionary saving into the capital supply schedule and clear it against firm capital demand. With persistent income risk and a no-borrowing constraint, the equilibrium real rate sits strictly below $1/\beta-1$, and a Gini of about $0.5$ falls out of the income chain alone. |
...
| Preview | Tutorial | Description |
|---|---|---|
| [<img src="heterogeneous-agents/endogenous-grid-points/figures/thumb.png" width="160">](heterogeneous-agents/endogenous-grid-points/figures/consumption-policy.png) | **[Buffer-Stock Saving by Endogenous Grid Points](heterogeneous-agents/endogenous-grid-points/)** | Carroll's (2006) reversal: fix the grid in next-period assets $a'$, recover today's consumption from the Euler equation, and read the implied current asset off the budget identity — one expectation and one interpolation per income state, no inner argmax. The coarse-grid policy lines up with a $900$-point reference to about $4\times 10^{-4}$ on the active asset range, and 50,000 simulated households deliver an average MPC near $0.23$, well above the perfect-foresight $\kappa^{\ast}\approx 0.04$, because liquidity constraints bind for a non-trivial mass. |
| [<img src="heterogeneous-agents/envelope-equation-iteration/figures/thumb.png" width="160">](heterogeneous-agents/envelope-equation-iteration/figures/consumption-policy.png) | **[Envelope-Equation Iteration for Buffer-Stock Saving](heterogeneous-agents/envelope-equation-iteration/)** | Maliar and Maliar (2013): use the envelope condition $W_a(a)=R\,\mathbb{E}_y\,u'(c(a,y))$ as the *update rule* for a fixed point on the marginal continuation value, not just as a theorem behind the Euler equation. The Euler step solves $u'(c)=\beta\,W_a(R a + y - c)$ by bisection at every state, so a single one-dimensional curve $W_a(a)$ summarises tomorrow's policy. EEI, EGP, and grid VFI agree on the same buffer-stock policy to about $10^{-2}$ on the active asset range; the simulated mean MPC of $0.22$ from a $0.10$ transfer is an order of magnitude above the perfect-foresight $\kappa^{\ast}\approx 0.04$ because the constraint binds for $3\%$ of the cross section. |
| [<img src="heterogeneous-agents/huggett-incomplete-markets/figures/thumb.png" width="160">](heterogeneous-agents/huggett-incomplete-markets/figures/bond-market.png) | **[Huggett Equilibrium and the Risk-Free Rate](heterogeneous-agents/huggett-incomplete-markets/)** | Huggett (1993) in continuous time, à la Achdou et al. (2022): a two-state Poisson income chain, a hard borrowing limit $a \geq \underline a$, and a single bond in zero net supply. The household block is a Hamilton-Jacobi-Bellman equation solved by an implicit upwind finite-difference scheme; the cross-sectional density solves the matching Kolmogorov Forward Equation $A^{\top} g = 0$; the equilibrium $r^{\ast}$ is the bisection root that makes aggregate desired bond holdings vanish. With incomplete insurance and precautionary buffer demand, the wedge $\rho - r^{\ast} \approx 0.015$ — about $30\%$ of the rate of time preference — is the price the economy charges for the missing state-contingent insurance contract; the working $2{,}000$-point grid lines up with a $6{,}000$-point reference to within $7\times 10^{-4}$ in $r^{\ast}$ and $1.6\times 10^{-3}$ in the savings policy on the active asset range. |
...
| Preview | Tutorial | Description |
|---|---|---|
| [<img src="dsge/rbc/figures/thumb.png" width="160">](dsge/rbc/figures/irf-tfp-shock.png) | **[RBC TFP Shocks and Capital Propagation](dsge/rbc/)** | Study how a favorable productivity shock is split between consumption, investment, and future capital in a representative-agent RBC economy. First-order log-linear perturbation with a Klein QZ cross-check; a 1% shock makes investment jump 3.2% while the nonlinear transition stays within 0.033 pp. |
