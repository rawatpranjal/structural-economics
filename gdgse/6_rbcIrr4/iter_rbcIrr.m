...
GNDSGE_DATA = zeros(17,GNDSGE_NPROB);
...
GNDSGE_DATA(:) = [repmat([shock_num;beta(:);sigma(:);alpha(:);delta(:);phi(:);Iss(:);shock_trans(:);z(:);z2(:); ],1,GNDSGE_NPROB);GNDSGE_TENSOR_shockIdx(:)';GNDSGE_TENSOR_K(:)';   ];
...
IterRslt.output_var_index = output_var_index;
        ...
        IterRslt.var_shock = v2struct(z,z2);
