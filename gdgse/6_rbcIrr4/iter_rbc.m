...
GNDSGE_LB = -1e20*ones(3,GNDSGE_NPROB);
GNDSGE_UB = 1e20*ones(3,GNDSGE_NPROB);
GNDSGE_LB(1,:)=0;
GNDSGE_UB(1,:)=GNDSGE_TENSOR_z(:)'.*GNDSGE_TENSOR_K(:)'.^alpha+(1-delta)*GNDSGE_TENSOR_K(:)';
GNDSGE_LB(2,:)=(1-delta)*GNDSGE_TENSOR_K(:)'+phi*Iss;
GNDSGE_UB(2,:)=GNDSGE_TENSOR_z(:)'.*GNDSGE_TENSOR_K(:)'.^alpha+(1-delta)*GNDSGE_TENSOR_K(:)';
GNDSGE_LB(3,:)=0;
GNDSGE_UB(3,:)=1.0;
...
if ~GNDSGE_USE_OLD_VEC
    ...
    if isfield(GNDSGE_OPTIONS.WarmUp,'GNDSGE_PROB') && size(GNDSGE_OPTIONS.WarmUp.GNDSGE_PROB.GNDSGE_SOL,1)==size(GNDSGE_SOL,1)
        ...
        v2struct(GNDSGE_OPTIONS.WarmUp.var_state);
        GNDSGE_SOL_interp=struct('form','MKL','breaks',{{[1:shock_num],K}},'Values',reshape(GNDSGE_OPTIONS.WarmUp.GNDSGE_PROB.GNDSGE_SOL,[],GNDSGE_SIZE_STATE{:}),'coefs',[],'order',[2,GNDSGE_INTERP_ORDER],'Method',[],'ExtrapolationOrder',[2,GNDSGE_EXTRAP_ORDER],'thread',NumThreads,'orient','curvefit');
        GNDSGE_SOL_interp=myinterp(GNDSGE_SOL_interp);
        GNDSGE_LB_interp=struct('form','MKL','breaks',{{[1:shock_num],K}},'Values',reshape(GNDSGE_OPTIONS.WarmUp.GNDSGE_PROB.GNDSGE_LB,[],GNDSGE_SIZE_STATE{:}),'coefs',[],'order',[2,GNDSGE_INTERP_ORDER],'Method',[],'ExtrapolationOrder',[2,GNDSGE_EXTRAP_ORDER],'thread',NumThreads,'orient','curvefit');
        GNDSGE_LB_interp=myinterp(GNDSGE_LB_interp);
        GNDSGE_UB_interp=struct('form','MKL','breaks',{{[1:shock_num],K}},'Values',reshape(GNDSGE_OPTIONS.WarmUp.GNDSGE_PROB.GNDSGE_UB,[],GNDSGE_SIZE_STATE{:}),'coefs',[],'order',[2,GNDSGE_INTERP_ORDER],'Method',[],'ExtrapolationOrder',[2,GNDSGE_EXTRAP_ORDER],'thread',NumThreads,'orient','curvefit');
