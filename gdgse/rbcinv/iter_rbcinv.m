function [IterRslt,IterFlag] = iter_rbcinv(GNDSGE_OPTIONS)
%% Add path
if ispc
...
assert(length(z)==10);
assert(length(v)==10);
assert(size(shock_trans,1)==10);
assert(size(shock_trans,2)==10);
...
GNDSGE_LB(1:1,:)=0;
GNDSGE_UB(1:1,:)=exp(GNDSGE_TENSOR_z(:)').*GNDSGE_TENSOR_K(:)'.^alpha+(1-delta)*GNDSGE_TENSOR_K(:)';
GNDSGE_LB(2:2,:)=(1-delta)*GNDSGE_TENSOR_K(:)'+phi*Iss;
GNDSGE_UB(2:2,:)=exp(GNDSGE_TENSOR_z(:)').*GNDSGE_TENSOR_K(:)'.^alpha+(1-delta)*GNDSGE_TENSOR_K(:)';
GNDSGE_LB(3:3,:)=0;
GNDSGE_UB(3:3,:)=1.0;
...
GNDSGE_DATA = zeros(129,GNDSGE_NPROB);
...
c_interp(:)=exp(GNDSGE_TENSOR_z(:)').*GNDSGE_TENSOR_K(:)'.^alpha+(1-delta)*GNDSGE_TENSOR_K(:)';
mu_interp(:)=0;
...
GNDSGE_DATA(:) = [repmat([shock_num;beta(:);sigma(:);alpha(:);delta(:);phi(:);Iss(:);shock_trans(:);z(:);v(:); ],1,GNDSGE_NPROB);GNDSGE_TENSOR_shockIdx(:)';GNDSGE_TENSOR_K(:)';   ];
...
IterRslt.output_var_index = output_var_index;
        ...
        IterRslt.var_shock = v2struct(z,v);
        ...
        if ~NoSave
            if IterSaveAll
                save(['IterRslt_rbcinv_' num2str(GNDSGE_Iter) '.mat']);
            else
                save(['IterRslt_rbcinv_' num2str(GNDSGE_Iter) '.mat'],'IterRslt');
            end
        end
    end
end
