...
"""
    ...
    report.add_solution_method(
        ...
        "The expected error scales as $1/\\sqrt{N}$ on a unimodal problem and degrades on nonconcave ones in proportion to the volume share of the global basin. "
        ...
        "It samples a Markov chain that proposes random moves and accepts them with a probability that depends on the change in objective and a slowly decreasing temperature. "
        "SciPy's `dual_annealing` combines a generalised-simulated-annealing global search with local refinement at each accepted move. "
        "The method has provable convergence to the global optimum under a logarithmic cooling schedule, but the constant is impractical and the practical schedule is heuristic.\n\n"
        "```text\n"
