...
Many economic objects are expectations of a function against a Gaussian density. The Euler equation in a dynamic-programming problem with an AR(1) productivity shock, the bid-shading integral in a Bayes-Nash auction, and the acquisition integral in Bayesian optimisation all sit on the same template: compute the expected value of some function $`f`$ when its argument is a normally distributed random variable. Monte Carlo can do this, but it converges at rate $`R^{-1/2}`$ in the number of draws, which is slow when $`f`$ is smooth.

Gauss-Hermite quadrature exploits the smoothness. It picks a small number of carefully placed nodes (the roots of the Hermite polynomials) and weights (derived from the same polynomials by Christoffel-Darboux), so that the resulting weighted sum reproduces the integral exactly when $`f`$ is a polynomial up to degree $`2N - 1`$ in $`N`$ nodes. For smooth non-polynomial integrands, the error decays geometrically in $`N`$. Three nodes already integrate a fourth-degree polynomial exactly. Ten nodes integrate $`\exp(-z^2/4)`$ to machine precision.
