...
References:
- Karush, W. (1939). Minima of Functions of Several Variables...
...
def main() -> None:
    ...
    # =========================================================================
    # Baseline: Lagrangian on the budget alone (fails on non-negativity)
    # =========================================================================
    ...
    # =========================================================================
    # Method 1: projected gradient onto the budget simplex
    # =========================================================================
    step = 0.25
    ...
    # ------------------------------------------------------------------
    # Figure 1 (1x2): feasible-region geometry
    #   Left  -- projected-gradient path on the budget simplex
    #   Right -- interior-point central path
    # ------------------------------------------------------------------
    fig1, (ax1a, ax1b) = plt.subplots(1, 2, figsize=(13, 6))

    for ax, color, title in [
        (ax1a, "tab:orange", fr"Projected gradient on the budget simplex (project 3 implicit, $x_3 = I - x_1 - x_2$)"),
        (ax1b, "tab:purple", "Interior-point central path as the barrier shrinks"),
    ]:
        triangle = Polygon(
        ...
        ax.plot([0.0, I_total, 0.0, 0.0], [0.0, 0.0, I_total, 0.0],
                color="tab:blue", linewidth=1.5, alpha=0.6)
        ax.plot(x_star[0], x_star[1], "*", color="tab:red", markersize=18,
                label=fr"$x^{{\ast}} = (2,\,1,\,0)$")
        ax.set_xlabel("Project 1 allocation $x_1$")
        ax.set_ylabel("Project 2 allocation $x_2$")
        ax.set_title(title, fontsize=9)
        ax.set_xlim(-0.2, I_total + 0.5)
        ax.set_ylim(-0.2, I_total + 0.5)
        ax.set_aspect("equal")

    # Left panel: projected gradient
    pg_arr = np.array(pg_history)
    ax1a.plot(pg_arr[:, 0], pg_arr[:, 1], "-", color="tab:orange", linewidth=1.0, alpha=0.7)
    n_show_pg = min(15, len(pg_arr))
    ax1a.plot(pg_arr[:n_show_pg, 0], pg_arr[:n_show_pg, 1],
              "o", color="tab:orange", markersize=5, label="Projected gradient iterate")
    ax1a.plot(pg_arr[0, 0], pg_arr[0, 1], "o", color="tab:gray", markersize=8,
              markeredgecolor="black", label=fr"Start $x_0 = (0.5,\,0.5,\,2.0)$")
    ax1a.text(I_total + 0.05, 0.05, "$x_3 = 0$\n(project 3 inactive)", fontsize=8, color="tab:purple")
    ax1a.legend(loc="upper right", fontsize=8)

    # Right panel: barrier central path
    barrier_arr = np.array([h[1] for h in barrier_history])
    ax1b.plot(barrier_arr[:, 0], barrier_arr[:, 1], "-", color="tab:purple",
              linewidth=1.5, alpha=0.7, label="Central path")
    for i, (t, x_t, _) in enumerate(barrier_history):
        ax1b.plot(x_t[0], x_t[1], "d", color="tab:purple", markersize=7)
    t0, x0_arr, _ = barrier_history[0]
    tend, xend, _ = barrier_history[-1]
    ax1b.annotate(fr"$t = {t0:.0e}$ (most interior)",
    ...
    ax1b.annotate(fr"$t = {tend:.0e}$ (near $x^{{\ast}}$)",
    ...
    fig1.tight_layout()
    save_figure(fig1, "figures/geometry.png", dpi=150)

    # ------------------------------------------------------------------
    # Figure 2 (1x2): KKT residuals over iterations / barrier values
    # ------------------------------------------------------------------
    fig2, (ax2a, ax2b) = plt.subplots(1, 2, figsize=(12, 5))

    pg_kkt = np.array(pg_kkt_trace)
    pg_iters_axis = np.arange(len(pg_kkt))
    ax2a.semilogy(pg_iters_axis, np.maximum(pg_kkt[:, 0], 1e-16), "-", color="tab:orange",
                  linewidth=1.5, label="Stationarity")
    ax2a.semilogy(pg_iters_axis, np.maximum(pg_kkt[:, 1], 1e-16), "-", color="tab:green",
                  linewidth=1.5, label="Primal feasibility")
    ax2a.semilogy(pg_iters_axis, np.maximum(pg_kkt[:, 3], 1e-16), "-", color="tab:purple",
                  linewidth=1.5, label="Complementarity")
    ax2a.set_xlabel("Iteration $k$")
    ax2a.set_ylabel("KKT residual")
    ax2a.set_title("Projected gradient: KKT residuals across iterations")
    ax2a.legend(loc="upper right", fontsize=9)

    barrier_kkt = np.array(barrier_kkt_trace)
    barrier_idx = np.arange(len(barrier_kkt))
    ax2b.semilogy(barrier_idx, np.maximum(barrier_kkt[:, 0], 1e-16), "d-", color="tab:orange",
                  markersize=5, linewidth=1.5, label="Stationarity")
    ax2b.semilogy(barrier_idx, np.maximum(barrier_kkt[:, 1], 1e-16), "d-", color="tab:green",
                  markersize=5, linewidth=1.5, label="Primal feasibility")
    ax2b.semilogy(barrier_idx, np.maximum(barrier_kkt[:, 3], 1e-16), "d-", color="tab:purple",
                  markersize=5, linewidth=1.5, label="Complementarity")
    ax2b.set_xticks(barrier_idx)
    ax2b.set_xticklabels([f"{t:.0e}" for t in barriers], rotation=45, fontsize=8)
    ax2b.set_xlabel("Barrier parameter $t$")
    ax2b.set_ylabel("KKT residual")
    ax2b.set_title("Interior point: KKT residuals along the central path")
    ax2b.legend(loc="upper right", fontsize=9)
    fig2.tight_layout()
    save_figure(fig2, "figures/kkt-residuals.png", dpi=150)

    # ------------------------------------------------------------------
    # Figure 3: shadow prices at the SLSQP optimum
    # ------------------------------------------------------------------
    ...
    width = 0.35
    ax3.bar(pos - width / 2, closed_form, width, color="tab:gray", alpha=0.7,
            label="Closed form")
    ax3.bar(pos + width / 2, slsqp_vals, width, color="tab:red", alpha=0.85,
            label="SLSQP recovery")
    ax3.set_xticks(pos)
    ax3.set_xticklabels(labels, fontsize=9)
    ax3.set_ylabel("Multiplier value")
    ax3.set_title("Shadow prices: closed form vs SLSQP recovery")
    ax3.axhline(0.0, color="black", linewidth=0.8, alpha=0.5)
    ax3.legend(loc="upper right", fontsize=9)
    for i, (cf, sl) in enumerate(zip(closed_form, slsqp_vals)):
        if abs(cf) > 1e-8 or abs(sl) > 1e-8:
            ax3.text(i, max(cf, sl) + 0.05, fr"$\lambda^{{\ast}} = {cf:.2f}$" if i == 0 else fr"${cf:.2f}$",
                     ha="center", fontsize=8)
    save_figure(fig3, "figures/shadow-prices.png", dpi=150)
    ...
    save_thumbnail("figures/geometry.png", "figures/thumb.png")
