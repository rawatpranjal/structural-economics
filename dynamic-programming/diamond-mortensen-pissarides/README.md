...
There are two computations. The report uses the nonlinear finite-state solution as a check on the log-linear tightness rule, then simulates unemployment from the nonlinear rule. The approximation gap is small in this calibration. The economic point is that the standard surplus calibration leaves little amplification to begin with.
...
Over this shock range, the log-linear rule tracks the nonlinear free-entry fixed point closely. The failure is economic: even the nonlinear rule moves tightness only about 1.72 times as much as productivity, well below the volatility of labor-market tightness emphasized by Shimer.
