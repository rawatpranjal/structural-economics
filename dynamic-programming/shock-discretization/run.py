...
def main() -> None:
    ...
    report.add_overview(
        "Many dynamic models begin with a familiar economic object: income or "
        "productivity is risky, and bad times tend to persist. A household deciding "
        "how much to save cares about whether a low income draw is likely to last. "
        "A planner in an RBC model cares about whether today's productivity shock "
        "also changes tomorrow's investment return. That is the economic question: "
        "what risk process should the model feed into choices and continuation "
        "values?\n\n"
        "The computational question is different. A computer needs a finite object "
        "inside the Bellman equation, so the continuous AR(1) shock has to become "
        "a small grid of states and a transition matrix. This tutorial asks how much "
        "of the original persistence and long-run risk survive that replacement.\n\n"
        "Tauchen and Rouwenhorst are two common answers. **Tauchen** is easy to see: "
        "put a grid over the Gaussian support and integrate conditional normal mass "
        "between cell midpoints. **Rouwenhorst** is less visual but targets the "
        "moments that matter for persistent shocks. With $\\rho \\approx 0.95$ and "
        "$N=7$, Tauchen overstates persistence by about a percentage point and the "
        "unconditional standard deviation by more than 20 percent, while Rouwenhorst "
        "matches both by construction. Those differences feed directly into the "
        "[consumption-savings](../consumption-savings/), [Aiyagari](../aiyagari/), "
        "and [RBC](../rbc/) tutorials that use this kind of shock."
    )
