...
def main() -> None:
    ...
    report.add_overview(
        "Suppose income is low this year. In a consumption-savings model, the "
        "household wants to know whether that low draw is likely to continue. In "
        "an RBC model, a productivity shock matters for investment because it also "
        "changes expected productivity tomorrow. The economics is about the shock "
        "process that choices and continuation values should face.\n\n"
        "The code cannot put a continuous AR(1) directly into a small Bellman "
        "equation. It needs a finite list of shock states and a transition matrix. "
        "The practical question is whether that finite Markov chain keeps the "
        "persistence and long-run variance that make the original process "
