...
def generate_product_data(n_markets: int = 100, seed: int = 42) -> pd.DataFrame:
    ...
    products = {
        ...
        "sugar":             [10.0, 1.0, 8.0, 6.0, 3.0],
        "fiber":             [1.0, 9.0, 2.0, 4.0, 7.0],
        "xi":                [0.5, 0.2, -0.3, 0.1, 0.4],
        "firm_id":           [1, 2, 1, 3, 2],       # Firms 1 & 2 are multi-product
    ...
    for t in range(n_markets):
        ...
        for j in range(J):
            ...
            # Endogeneity: firms charge more for high-xi products
            markup = rng.uniform(0.3, 0.8) + 0.2 * products["xi"][j]
            price = mc * (1 + markup)
            rows.append({
                "market_id": t,
...
""")
    ...
    table_data = {
        "Parameter": [r"alpha", r"beta_sugar", r"beta_fiber", r"beta_const"],
