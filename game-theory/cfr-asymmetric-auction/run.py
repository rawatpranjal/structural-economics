...
def main() -> None:
    ...
    report.add_overview(
        ...
        "The tutorial implements vanilla CFR on the asymmetric game and gives it three "
        "diagnostics. The symmetric closed form anchors the implementation when both "
        "value distributions are made equal. The continuous asymmetric BNE itself is "
        "recovered independently by solving the Marshall, Meurer, Richard, and Stromquist "
        "boundary-value problem on the inverse bid functions, and overlaid on the CFR "
        "result. Exploitability of the average strategy on the discretized game is the "
        "no-deviation diagnostic, the same idea as the bid-grid deviation check in the "
        "existing first-price auction tutorial."
    )
...
""")
    ...
    report.add_solution_method(
        ...
        "bounds into a global average regret bound on the time-averaged strategy.\n\n"
        "The tightest theoretical guarantee that the time-averaged strategy converges to "
        "a Nash equilibrium holds in two-player zero-sum games. The first-price auction "
        "is general-sum from the bidders' point of view, so CFR's convergence here is "
        "empirical rather than guaranteed by the textbook bound. Two checks pin it down "
        "on this run anyway: the MMRS BNE recovered in the Results section, and the "
        "exploitability of the average strategy logged across iterations.\n\n"
        "### Worked example\n\n"
        "To see CFR step by step, work one iteration on a tiny bid grid "
        "$B = \\lbrace 0, 0.5 \\rbrace$ for the weak bidder's high-value type $v = 1$, "
        "against an opponent who plays uniformly over those two bids at every type. The "
        "marginal opponent bid distribution is $q = (0.5, 0.5)$. Win probabilities under "
        "uniform tie-break are $w(0) = 0.25$ and $w(0.5) = 0.75$. Expected payoffs at "
        "$v = 1$ are $(1 - 0) \\cdot 0.25 = 0.25$ at the low bid and "
        "$(1 - 0.5) \\cdot 0.75 = 0.375$ at the high bid. The uniform current strategy "
        "mixes these into a strategy-mixture value of $0.3125$. Regret for the low bid "
        "is $0.25 - 0.3125 = -0.0625$ and regret for the high bid is "
        "$0.375 - 0.3125 = +0.0625$. Regret matching on $\\max(R, 0)$ then sets the next "
        "strategy at this information set to put all weight on the high bid, which is "
        "exactly the symmetric BNE bid for type $v = 1$. The full algorithm repeats this "
        "calculation independently at every type for both bidders; the time-averaged "
        "strategy across many iterations on a finer bid grid is what converges to the "
        "asymmetric BNE.\n\n"
        "### Algorithm\n\n"
        "```text\n"
        ...
        "```\n\n"
        "The exploitability $\\varepsilon(\\bar{\\sigma})$ from the Equations section is "
        "the convergence diagnostic. It is logged at a logarithmic grid of iteration "
        "counts so the decay shows up cleanly on a log-log plot."
    )
    ...
    report.add_results(
        ...
        "rival often holds a higher value, so shading too much loses too many auctions. "
        ...
        "functions track the BNE within bid-grid spacing across the full type range. The "
        "agreement between two independent computations is the convergence test that "
        "exploitability alone cannot give."
    )
    report.add_figure(
